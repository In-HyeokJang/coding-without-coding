...
## 6. 회차별 날짜 자동 공개 ⭐ (기법 기록 · 지금은 안 씀)

> **2026-08-11 에 걷어냈음.** 회차를 전부 열어두기로 해서 (참가자가 언제 오든 1~6회차를 다 볼 수 있게)
> `.github/workflows/auto-open-sessions.yml` 을 지웠다. 아래는 기법 자체의 기록이다.
>
> **다시 쓸 거면 함정 하나를 먼저 알아야 함 —**
> 워크플로가 기본 `GITHUB_TOKEN` 으로 푸시하면 **다른 워크플로가 트리거되지 않는다.**
> 그래서 "푸시되면 deploy.yml 이 이어서 돈다" 는 아래 4번이 **실제로는 안 돌았다.**
> draft 는 풀렸는데 사이트는 그대로라 회차 문서가 404 였다.
> 이어서 배포까지 가게 하려면 `deploy.yml` 에 `workflow_run` 트리거를 달거나, 푸시에 개인 토큰(PAT)을 써야 한다.
...
### 지금 회차 파일에 남아 있는 `openDate`

상시 공개로 바꾼 뒤로 **여닫는 데는 안 쓴다.** 그 회차의 모임 날짜 기록으로만 남겨뒀다.
...
> 자동 공개를 쓰던 동안에는 6회차만 **08-11 로 당겨** 5회차와 같은 날 열었다.
> 그 장치를 걷어내면서 실제 모임일로 되돌렸다.

시즌 2 준비할 때: **위 워크플로우 파일은 지워졌으므로 다시 만들어야 함.** 그리고 만들기 전에 절 맨 위 경고를 먼저 읽을 것 — 그대로 되살리면 draft 만 풀리고 배포가 안 돈다.

애초에 상시 공개로 갈 거면 이 절은 통째로 건너뛰면 됨. 회차 파일에 `draft` 를 안 넣으면 그게 상시 공개다.
